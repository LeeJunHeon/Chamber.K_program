...
class SputterProcessController(QObject):
    ...
    def __init__(self, mfc_controller, dc_controller, rf_controller, faduino_controller):
        ...
        self.gas_valve_button = None        # 선택된 가스에 따라 제어할 Faduino 버튼 이름 (예: 'Ar_Button')
    ...
    def _prepare_steps(self, params):
        ...
        if params.get('use_g1', False):
            self._steps.append(("FADUINO", "S1_button", True, "Gun Shutter 1(S1)을 엽니다."))

        # [수정] G2 체크박스가 선택된 경우에만 S2 Shutter 열기 단계를 추가
        if params.get('use_g2', False):
            self._steps.append(("FADUINO", "S2_button", True, "Gun Shutter 2(S2)를 엽니다."))
